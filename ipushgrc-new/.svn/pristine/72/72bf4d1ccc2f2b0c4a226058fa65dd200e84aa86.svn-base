package com.pushworld.ipushgrc.ui.wfrisk.p070;
...
/**
 * 体系文件编辑权限设置!! 即有些客户要求指定什么体系文件只能什么人编辑!! 但大多数客户不需要这种需求! 而需要查询权限!
 * 机场财务需要加上编辑权限，故加上以备用【李春娟/2014-09-16】
 * @author xch
 *
 */
public class CmpFileEditPermitWKPanel extends AbstractWorkPanel implements ActionListener {
	...
	public void initialize() {
		try {
			HashVO[] hvs = UIUtil.getHashVoArrayByDS(null, "select * from pub_datapolicy where name='流程文件编辑策略'"); ////
			if (hvs == null || hvs.length <= 0) {
				this.add(new JLabel("没有定义名为【流程文件编辑策略】的数据权限策略,请定义之!")); //
				return; //
			}
			...
			billList.getTitleLabel().setText("流程文件编辑策略"); //
