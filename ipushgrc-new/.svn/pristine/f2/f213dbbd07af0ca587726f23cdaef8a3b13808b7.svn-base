...
public class UnFormatPrint1 extends MultiLevelReportDataBuilderAdapter {
	...
	public HashVO[] buildReportData(HashMap _condition) throws Exception {
		...
		sb.append("Select sfile.Name 合同名称 ,dept.Name 创建机构,combo.name 合同性质 ,busi.Name 业务类型 ," + "tfile.Name 合同属性,sfile.createdate 创建日期 " + "  "
				+ "From lbs_unstdfile sfile ,contractbusi busi ,lbs_stdfile_type tfile ,pub_corp_dept dept ,pub_comboboxdict combo "
				+ "Where 1=1 And sfile.createorg = dept.Id And sfile.Type = busi.Id And sfile.busiid = tfile.Id " + "and combo.type= '合同_合同性质2' and combo.id = sfile.property ");
