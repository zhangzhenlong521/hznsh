...
public class CmpfileReportBuilderAdapter extends MultiLevelReportDataBuilderAdapter {
	...
	public HashVO[] buildReportData(HashMap condition) throws Exception {
		...
		RefItemVO ref_versionno = (RefItemVO) condition.get("obj_versionno");//版本号
		String ictypeid = (String) condition.get("ictypeid");//内控要素
		String productid = (String) condition.get("productid");//对应产品

		TBUtil tbutil = new TBUtil();
		StringBuffer sb_sql = new StringBuffer("select id,cmpfiletype 文件分类,filestatename 文件状态, blcorpid 所属机构,bsactid 业务活动,ictypeid 内控要素,publishdate 发布日期  from v_cmp_cmpfile where 1=1 ");
		...
		if (ref_versionno != null) {
			String str_cons = ref_versionno.getHashVO().getStringValue("querycondition"); //取得查询条件itemkey
			str_cons = tbutil.replaceAll(str_cons, "{itemkey}", "versionno"); //替换其中的特殊符号!为实际字段名!
			sb_sql.append(" and " + str_cons);
		}
		...
		if (productid != null && !"".equals(productid)) {
			sb_sql.append(" and productid in(" + tbutil.getInCondition(productid) + ") ");
		}
