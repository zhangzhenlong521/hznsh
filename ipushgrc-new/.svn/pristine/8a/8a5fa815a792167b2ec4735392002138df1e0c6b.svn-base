 ...
 * @since 2011.12.19
 */
public class FormatPrint2 extends MultiLevelReportDataBuilderAdapter{

	@Override
	public HashVO[] buildReportData(HashMap _condition) throws Exception {
		StringBuffer sb = new StringBuffer();
		sb.append("Select de.Name 机构名称,ls.Name 合同名称,ld.pdate 打印日期,ld.pcount 打印张数 " + 
				  "From lbs_dept_print_contact ld ,pub_corp_dept de , V_LBS_STAND_CHECK_FILE ls " +
				  "Where ld.dept_id = de.Id And ld.contact_id = ls.Id and ld.type = '1'" 
				);
		if(_condition.get("DEALDOC_NAME")!=null && !_condition.get("DEALDOC_NAME").equals("")) //合同名称
			sb.append(" and ld.contact_id in " + getInConditionWtihQuote((String) _condition.get("DEALDOC_NAME"))) ;
		if(_condition.get("SENDORG")!=null && !_condition.get("SENDORG").equals("")) //发送机构
	...
	public String[] getGroupFieldNames() {
		return new String[]{"机构名称","合同名称","打印日期"};
	}
	// 求和区域
	@Override
