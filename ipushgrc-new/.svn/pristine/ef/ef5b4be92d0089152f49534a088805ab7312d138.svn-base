...
/**
 * 风险评估申请-建议修改的风险点所在环节的选择
 * @author lcj
 *
 */
public class SuggestEditRiskWLTAction implements WLTActionListener, ActionListener {
	...
	public void actionPerformed(WLTActionEvent _event) throws Exception {
		...
		listdialog = new BillListDialog(cardPanel.getParent(), "请选择建议修改的风险点", "V_RISK_PROCESS_FILE_CODE3");
		...
		CardCPanel_TextArea textarea = (CardCPanel_TextArea) cardPanel.getCompentByKey("sug_editrisks");
	...
	private void afterClose() {
		...
		String sug_editrisks = billVO.getStringValue("sug_editrisks");
		StringBuffer sb_desc = new StringBuffer();
		int count = 1;
		for (int i = 0; i < wfrisk.size(); i++) {
			if (i == 0 && (sug_editrisks != null && !"".equals(sug_editrisks.trim()))) {
				sb_desc.append(sug_editrisks);
				if (!sug_editrisks.endsWith("\n")) {
					sb_desc.append("\n");
				}
			}
			if (i == 0) {
		...
		try {
			...
			if (sb_desc.length() > 0) {
				cardPanel.setRealValueAt("sug_editrisks", sb_desc.toString());
			}
		} catch (Exception e1) {
