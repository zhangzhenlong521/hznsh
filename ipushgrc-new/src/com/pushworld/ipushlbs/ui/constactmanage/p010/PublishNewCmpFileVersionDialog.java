...
public class PublishNewCmpFileVersionDialog extends BillDialog implements ActionListener {
	...
	private int bigversion,smallversion;
	public PublishNewCmpFileVersionDialog(Container _parent, String versionno) {
		super(_parent, "发布新版本", 320, 160);
		int inde = versionno.indexOf(".");
		
		if(inde>0){
			bigversion = Integer.parseInt(versionno.substring(0, versionno.indexOf(".")));
			smallversion = Integer.parseInt(versionno.substring(versionno.indexOf(".") + 1));
		}else{
			bigversion = Integer.parseInt(versionno);
		}
