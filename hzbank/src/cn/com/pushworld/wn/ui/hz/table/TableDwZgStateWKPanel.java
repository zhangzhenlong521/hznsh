    ...
    public void getQnELoanDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(wg.g) hs from hzdb.s_qwyt_dwzg_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(1,"yyyyMM")+" wg on upper(ry.f)=upper(wg.c)\n" +
                    "where wg.c is not null group by ry.g) sy\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs from hzdb.s_qwyt_dwzg_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(0,"yyyyMM")+" wg on upper(ry.f)=upper(wg.c)\n" +
                    "where wg.c is not null group by ry.g) dy on sy.code=dy.code\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs from hzdb.s_qwyt_dwzg_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getYearMonth()+" wg on upper(ry.f)=upper(wg.c)\n" +
