...
public class TableDwZgStateWKPanel extends AbstractWorkPanel implements ActionListener {
    ...
    public void initialize() {
        billCellPanel=new BillCellPanel("s_table_dwzg" ,null, null,true, false,true);
        try{
            getTablename();
    ...
    public void getQnELoanDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(wg.g) hs from hzdb.s_qwyt_dwzg_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(1,"yyyyMM")+" wg on upper(ry.f)=upper(wg.f)\n" +
                    "where wg.f is not null group by ry.g) sy\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs from hzdb.s_qwyt_dwzg_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(0,"yyyyMM")+" wg on upper(ry.f)=upper(wg.f)\n" +
