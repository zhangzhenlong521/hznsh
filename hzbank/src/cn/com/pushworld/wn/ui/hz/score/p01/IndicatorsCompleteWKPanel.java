...
public class IndicatorsCompleteWKPanel extends AbstractWorkPanel implements BillListSelectListener{
    ...
    private void CountDate(BillListPanel listPanel) {
        try{
            String [] col=UIUtil.getStringArrayFirstColByDS(null ,"select targetname from  hzdb.sal_person_check_auto_score where datadate='"+datadate+"' and targetname not like '网格%' group by targetname");
            ...
            HashVO [] vos=UIUtil.getHashVoArrayByDS(null,"select a.*,b.money from(SELECT checkeduser,deptname,checkedusername, "+colsb.toString()+"" +
                    ...
                    "hzdb.sal_person_check_auto_score "+sbSql.toString()+" and datadate='"+datadate+"' and targetname not like '网格%')a left join hzdb.v_pub_user_post_1 b on a.checkeduser=b.userid)\n" +
                    "  PIVOT(SUM(money)\n" +
                    "   FOR targetname IN("+zdsb.toString()+"))) a left join (select checkeduser,sum(money) money from hzdb.sal_person_check_auto_score where datadate='"+datadate+"' and targetname not like '网格%' group by checkeduser)\n" +
            ...
            templetVO.setTempletname("个人指标汇总");
