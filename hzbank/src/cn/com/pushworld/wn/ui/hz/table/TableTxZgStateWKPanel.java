    ...
    public void getQnyDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(ry.g) hs from hzdb.s_qwyt_txry_202012 ry left join hzdb.s_loan_qnyyx_"+DateUIUtil.getSDateMonth(1,"yyyyMM")+" qny on upper(ry.i)=upper(qny.f) \n" +
                    ...
                    "left join(\n" +
                    "select ry.g code,count(ry.g) hs from hzdb.s_qwyt_txry_202012 ry left join hzdb.s_loan_qnyyx_"+DateUIUtil.getSDateMonth(0,"yyyyMM")+" qny on upper(ry.i)=upper(qny.f) \n" +
            ...
            for(int i=0;i<date.length;i++){
                for(int j=0;j<date[i].length;j++){
                    ...
                    billCellPanel.setBackground("153,153,255",i+4,44);
                }
            }
        }catch (Exception e){
    ...
    public void getQnELoanDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(wg.g) hs from hzdb.s_qwyt_txry_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(1,"yyyyMM")+" wg on upper(ry.i)=upper(wg.c)\n" +
                    "where wg.c is not null group by ry.g) sy\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs from hzdb.s_qwyt_txry_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getSDateMonth(0,"yyyyMM")+" wg on upper(ry.i)=upper(wg.c)\n" +
                    "where wg.c is not null group by ry.g) dy on sy.code=dy.code\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs from hzdb.s_qwyt_txry_202012 ry left join hzdb.s_loan_esign_"+DateUIUtil.getYearMonth()+" wg on upper(ry.i)=upper(wg.c)\n" +
            ...
            for(int i=0;i<date.length;i++){
                for(int j=0;j<date[i].length;j++){
                    ...
                    billCellPanel.setBackground("102,255,102",i+4,35);
                }
            }
        }catch (Exception e){
    ...
    public void getLoanDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(wg.g) hs,round(sum(wg.dkye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join hzdb.Grid_Data_"+DateUIUtil.getSymDateMonth()+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.dkye>0 group by ry.g) sy\n" +
                    "left join(\n" +
                    "select ry.g code,count(wg.g) hs,round(sum(wg.dkye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join "+tablename+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.dkye>0 group by ry.g) dy on sy.code=dy.code\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs,round(sum(wg.dkye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join hzdb.Grid_Data_"+DateUIUtil.getYearYmTime()+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.dkye>0 group by ry.g) nc on sy.code=nc.code\n" +
                    "left join(\n" +
                    "select g code,count(g) hs from hzdb.s_qwyt_txry_202012 group by g) zj on sy.code=zj.code) order by to_number(fgm) desc)\n" +
                    "union all\n" +
            ...
            for(int i=0;i<date.length;i++){
                for(int j=0;j<date[i].length;j++){
                    ...
                    billCellPanel.setBackground("0,153,153",i+4,26);
                }
            }
        }catch (Exception e){
    ...
    public void getCkDate(){
        try{
            String date[][]=UIUtil.getStringArrayByDS(null,"select * from(\n" +
                    ...
                    "select ry.g code,count(wg.g) hs,round(sum(wg.ckye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join hzdb.Grid_Data_"+DateUIUtil.getSymDateMonth()+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.ckye>1000 group by ry.g) sy\n" +
                    "left join(\n" +
                    "select ry.g code,count(wg.g) hs,round(sum(wg.ckye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join "+tablename+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.ckye>1000 group by ry.g) dy on sy.code=dy.code\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.e) hs,round(sum(wg.ckye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join hzdb.Grid_Data_"+DateUIUtil.getYearYmTime()+" wg on upper(ry.i)=upper(wg.g)\n" +
                    ...
                    "where wg.ckye>1000 group by ry.g) dy on sy.code=dy.code\n" +
                    "left join\n" +
                    "(select ry.g code,count(wg.g) hs,round(sum(wg.ckye)/10000,2) num from hzdb.s_qwyt_txry_202012 ry left join hzdb.Grid_Data_"+DateUIUtil.getYearYmTime()+" wg on upper(ry.i)=upper(wg.g)\n" +
                    "where wg.ckye>1000 group by ry.g) nc on sy.code=nc.code\n" +
                    "left join(\n" +
                    "select g code,count(g) hs from hzdb.s_qwyt_txry_202012 group by g) zj on sy.code=zj.code)");
            for(int i=0;i<date.length;i++){
                for(int j=0;j<date[i].length;j++){
                    ...
                    billCellPanel.setBackground("255,102,255",i+4,13);
                }
            }
        }catch (Exception e){
