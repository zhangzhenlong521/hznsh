 ...
 * create by 2014-5-7
 */
public class BenDeptIndexPageReport implements DeskTopNewsDataBuilderIFC2 {
	...
	public HashVO[] getNewData(String loginUserCode, HashMap param) throws Exception {
		...
		if ("部门定量".equals(targettype) || "部门".equals(targettype)) {
			hvo = dmo.getHashVoArrayByDS(null, "select '完成值' 名称,t1.checkdate 日期,t1.reportvalue 值 from sal_dept_check_score t1 left join sal_target_list t2 on t1.targetid = t2.id left join sal_target_check_log t3 on t1.logid = t3.id where t2.checkeddept='" + ServerEnvironment.getCommDMO().getCurrSession().getLoginUserPKDept() + "' and t2.name='" + targetName + "' order by t3.checkdate");
