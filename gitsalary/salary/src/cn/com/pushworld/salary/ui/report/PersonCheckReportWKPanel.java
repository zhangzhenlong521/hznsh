...
public class PersonCheckReportWKPanel extends AbstractWorkPanel implements ActionListener {
	...
	private String[] types = new String[] { "员工定性指标" };
	private String[] types_nocou = new String[] { "员工考核分数" };
	private String reportname = "员工考核汇总";
	private String[] v_count_type = new String[] { "合计", "平均" };
	private String[] h_count_type = new String[] { "合计", "平均" };
	private static final Logger logger = WLTLogger.getLogger(PersonCheckReportWKPanel.class);
	...
	public void actionPerformed(ActionEvent e) {
		try {
			if (e.getSource() == billQueryPanel) {
				...
				HashVO[] hvs_log = UIUtil.getHashVoArrayByDS(null, "select id, checkdate from sal_target_check_log where status='考核结束' " + "and checkdate>='" + month_start + "' and checkdate<='" + month_end + "' order by checkdate");
				...
				HashVO[] hvs = ifc.getPersonCheck(checkids, types, types_nocou);
	...
	private BillCellVO getBillCellItemVOs(HashVO[] _hashVOs, String[] checkdates) {
		...
		for (int j = 0; j < types_nocou.length; j++) {
			for (int m = 0; m < _hashVOs.length; m++) {
				HashVO hvo = _hashVOs[m];
				for (int i = 0; i < checkdates.length; i++) {
					cellItemVOs[m + 2][title_len + i + types_all + checkdates.length * j].setCellvalue(hvo.getStringValue("result_b" + i + "_" + j, ""));
				}
			}
		}
