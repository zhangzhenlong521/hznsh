...
public class DataInterfaceTransferWKPanel extends AbstractWorkPanel implements BillListSelectListener, ChangeListener, ActionListener {
	...
	private void showHistory(String _selectdate) throws Exception {
		...
		String lasttime = billVO.getStringValue("lasttime");
		if (lasttime == null || lasttime.equals("")) {
			MessageBox.show(parentpanel, "该接口数据还没有进行同步");
			return;
		}
		...
		billListPanel.QueryDataByCondition("logid in (select id from sal_convert_ifcdata_log where exectype ='数据转换' and state ='成功' and reportname =(select iname from sal_data_interface_main where id ='" + iname + "') and datadate like '%" + currtime + "%' )");
	...
	public void stateChanged(ChangeEvent e) {
		...
		listPanel.QueryDataByCondition("logid in (select id from sal_convert_ifcdata_log where exectype ='数据转换' and state ='成功' and reportname =(select iname from sal_data_interface_main where id ='" + iname + "') and  datadate like '%" + exc + "%' )");
	...
	private void onConvert() {
		...
		try {
			...
			String uid = JOptionPane.showInputDialog(parentpanel, "请输入要手工同步的日期(格式:YYYY-MM-DD)", tbUtil.getCurrDate());
			if (tbUtil.isEmpty(uid)) {
				return;
			}
			ifc.convertIFCDataToReportByHand(vo.getStringValue("id"), uid);
			MessageBox.show(this, "执行成功!");
