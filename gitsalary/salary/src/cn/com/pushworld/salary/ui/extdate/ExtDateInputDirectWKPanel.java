package cn.com.pushworld.salary.ui.extdate;
 ...
 * create by 2013-9-22
 */
public class ExtDateInputDirectWKPanel extends AbstractWorkPanel implements ActionListener {
	private WLTTabbedPane maintab = null;
	private SalaryServiceIfc ifc = null;
	private String checkdate = null;

	public ExtDateInputDirectWKPanel() {

	}

	public ExtDateInputDirectWKPanel(String checkdate) {
	...
	public JPanel getFirstPanel() {
		...
		try {
			BillCellVO[] vos = getIfc().getExtDateDirect(checkdate, new String[] { ClientEnvironment.getCurrLoginUserVO().getBlDeptId() });
			if (vos != null && vos.length > 0) {
				for (int i = 0; i < vos.length; i++) {
					BillCellPanel bc = new BillCellPanel();
					if (checkdate == null && vos[i].getSeq() != null) {
						getSouthPanel(bc);
					}
	...
	public BillListPanel getSecondPanel() {
		...
		btn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					checkdate = svo.getStringValue("checkdate");
	...
	private void onSubmit(BillCellPanel bc, String state) {
		...
		try {
			if (items != null && items.length > 0) {
				for (int i = 0; i < items.length; i++) {
					for (int j = 0; j < items[i].length; j++) {
						if ("value".equals(items[i][j].getCellkey())) {
							if (items[i][j].getCustProperty("id") != null && !"".equals(items[i][j].getCustProperty("id"))) {
								UpdateSQLBuilder usb = new UpdateSQLBuilder("sal_quantifytargetdate");
								usb.putFieldValue("finavalue", items[i][j].getCellvalue());
								usb.setWhereCondition("id=" + items[i][j].getCustProperty("id"));
								if ("已提交".equals(state)) {
									usb.putFieldValue("state", state);
								}
								sql.add(usb.getSQL());
							} else {
								InsertSQLBuilder isb = new InsertSQLBuilder("sal_quantifytargetdate");
								String id = UIUtil.getSequenceNextValByDS(null, "S_SAL_QUANTIFYTARGETDATE");
								isb.putFieldValue("id", id);

								if ("1".equals(items[i][j].getCustProperty("targettype"))) {
									isb.putFieldValue("targettype", "部门定量指标");
								} else {
									isb.putFieldValue("targettype", "员工定量指标");
								}
								isb.putFieldValue("finavalue", items[i][j].getCellvalue());
								isb.putFieldValue("checkdate", bc.getBillCellVO().getDescr());
								isb.putFieldValue("targetid", items[i][j].getCustProperty("targetid") + "");
								isb.putFieldValue("targetcode", items[i][j].getCustProperty("targetcode") + "");
								isb.putFieldValue("state", state);
								sql.add(isb.getSQL());
								items[i][j].setCustProperty("id", id);
							}
						}
					}
				}
			...
			if (sql.size() > 0) {
				...
				if ("已提交".equals(state)) {
					bc.getTable().putClientProperty("issubmit", "Y");
					bc.repaint();
	...
	class MyUI extends MultiSpanCellTableUI {
		...
		private void paintCell(Graphics g, Rectangle cellRect, int row, int column) {
			...
			if ("Y".equals(table.getClientProperty("issubmit"))) {
				...
				double a = table.getWidth()/2 - 50; //.getVisibleRect().getCenterX() // 这样就不会动了
