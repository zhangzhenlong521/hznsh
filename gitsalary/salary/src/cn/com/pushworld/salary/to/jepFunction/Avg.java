package cn.com.pushworld.salary.to.jepFunction;
...
import cn.com.pushworld.salary.ui.target.p001.AllDeptTargetWKPanel;
		...
		if (obj.length > 0 && obj[0] instanceof HashVO[]) {
			...
			for (int i = 0; i < obj.length; i++) {
				try {
					...
					WLTLogger.getLogger(AllDeptTargetWKPanel.class).error("传入值转换成Float报错。" + Avg.class.getName(), ex);
				}
			}
			if (obj.length == 0) {
				f = 0;
