...
//		return sb.toString();
//	}
	// 把数字转换成英文编码  [zzl 2018-1-18] 新增此方法与excel列头一直
	public String convertIntColToEn(int _index,Boolean falg) {
		StringBuffer sb = new StringBuffer();
		int in = _index / 26;
		if (in == 0) { // 如果是最后一层
			if(falg){
				char ccc = (char) (65-2 + _index);
				sb.append(ccc);
			}else{
				char ccc = (char) (65 - 1 + _index);
				sb.append(ccc);
			...
			if(_index!=26 && _index % 26!=0){
				sb.insert(0, convertIntColToEn(in,false));
			}else if(_index!=26 && _index % 26==0){
				sb.insert(0, convertIntColToEn(in,true));
			}
		}
		return sb.toString();
	}
