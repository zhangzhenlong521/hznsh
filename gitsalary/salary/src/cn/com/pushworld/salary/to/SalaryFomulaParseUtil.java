...
import cn.com.pushworld.salary.to.jepFunction.AbstractPostfixMathCommand;
import cn.com.pushworld.salary.to.jepFunction.Avg;
import cn.com.pushworld.salary.to.jepFunction.Excel;
import cn.com.pushworld.salary.to.jepFunction.GetHashVOByTable;
import cn.com.pushworld.salary.to.jepFunction.GetSqlInCondition;
import cn.com.pushworld.salary.to.jepFunction.IF;
import cn.com.pushworld.salary.to.jepFunction.MatchByKey;
import cn.com.pushworld.salary.to.jepFunction.createMap;
...
public class SalaryFomulaParseUtil {
	...
	*/
	...
	protected Object onExecute(HashVO _factorHashVO, HashVO _baseDataHashVO, int _level, StringBuffer rtStr) throws Exception {
		...
		if (rtStr != null) {
			String st = "";
			if (obj instanceof String || obj instanceof Number || obj instanceof Integer) {//以前这里没有判断Integer，导致if嵌套使用重复因子会报错【李春娟/2017-04-19】
			...
			if (str[i].contains("[")) {
				...
				if ("传入数据".equals(sourceConfigName)) { //如果配置的是[传入数据.checkeddate].表示直接调用传入值的某一个字段。
					...
					if (factorCalcValue.containsKey(factorName)) { //判断是否已经放入缓存
						...
						if (ooo instanceof Number || ooo instanceof Integer || ooo instanceof String) {//以前这里没有判断Integer，导致if嵌套使用重复因子会报错【李春娟/2017-04-19】
							if (ooo instanceof Number || ooo instanceof Integer) {
								ooo = new BigDecimal(String.valueOf(ooo)); //by haoming 2016-01-27 double会科学计数法//这里以前加上了.toString()，导致重复获取值时数字变成了字符串，在用"<"或">"比较会报错【李春娟/2017-04-19】
							}
							_rtsb.append(factorName + " = " + String.valueOf(ooo) + "\r\n");
						}
					} else {
