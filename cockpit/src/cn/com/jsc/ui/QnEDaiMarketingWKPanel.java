 ...
 * 黔农E贷排名
 */
public class QnEDaiMarketingWKPanel {
    ...
    public JPanel getJLabel() {
        try {
            ...
            String [][] ckpm=service.getQnyEDKLvCount();
            panel.setLayout(null); //
            panel.setPreferredSize(new Dimension(1000, 1000)); //
            label_1 = new JLabel("<html><h3><font color='Black' size=6>"+DateUIUtil.getDateMonth(1,"yyyy年MM月dd日")+"客户经理营销黔农E贷排名</font></h3></html>");//
            ...
            for(int i=1;i<4;i++){
                ...
                label_4= new JLabel("<html><font color='#191970' size=5>营销户数("+ckpm[i-1][1]+"户)</font></html>");
                ...
                label_6= new JLabel(UIUtil.getImage("office_078.gif"));
            ...
            for(int i=1;i<4;i++){
                ...
                label_5.setBounds(330, a+50, 100, 50);
                label_6= new JLabel(UIUtil.getImage("zt_018.gif"));
                label_6.setBounds(400, a+50, 100, 50);
                panel.add(label_2);
                panel.add(label_3);
                panel.add(label_4);
            ...
            timer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    try{
                        System.out.println(">>>>>>>>>>>客户经理营销黔农E贷排名任务开始"+formatTemp.format(new Date()));
                        CockpitServiceIfc service = (CockpitServiceIfc) UIUtil
                                .lookUpRemoteService(CockpitServiceIfc.class);
                        ...
                        if(dk==null){
                            System.out.println(">>>>>>>>>>>客户经理营销黔农E贷排名退出任务"+formatTemp.format(new Date()));
                            return;
                        }else{
                            //zzl 记录修改的中间表
                            String count=UIUtil.getStringValueByDS(null,"select name from s_count where name='客户经理营销黔农E贷排名' and dates='"+DateUIUtil.getDateMonth(0,"yyyyMMdd")+"'");
                            if(count==null){
                                JLabel [][] jLabels=new JLabel[6][4];
                                JLabel JLabel= (javax.swing.JLabel) panel.getComponents()[0];
                                JLabel.setText("<html><h3><font color='Black' size=6>"+DateUIUtil.getDateMonth(1,"yyyy年MM月dd日")+"客户经理营销黔农E贷排名</font></h3></html>");
                                int row=0;
                                int col=0;
                                ...
                                for(int i=1;i<4;i++){
                                    ...
                                    jLabels[i-1][2].setText("<html><font color='#191970' size=5>营销户数("+ckpm[i-1][1]+"户)</font></html>");
                                ...
                                UIUtil.executeUpdateByDS(null,"Update s_count set dates='"+DateUIUtil.getDateMonth(0,"yyyyMMdd")+"' where name='客户经理营销黔农E贷排名'");
                                panel.updateUI();
                                System.out.println(">>>>>>>>>>>客户经理营销黔农E贷排名退出任务"+formatTemp.format(new Date()));
                            }else{
                                System.out.println(">>>>>>>>>>>客户经理营销黔农E贷排名退出任务"+formatTemp.format(new Date()));
