 ...
 * 客户经理清收不良贷款排名
 */
public class BadCollectWKPanel {
    ...
    public JPanel getJLabel() {
        try {
            ...
            String [][] ckpm=service.getqsBlDKLvCount();
            panel.setLayout(null); //
            panel.setPreferredSize(new Dimension(1000, 1000)); //
            label_1 = new JLabel("<html><h3><font color='Black' size=6>"+DateUIUtil.getDateMonth(1,"yyyy年MM月dd日")+"客户经理清收不良贷款排名</font></h3></html>");//
            ...
            for(int i=1;i<4;i++){
                ...
                label_4= new JLabel("<html><font color='#191970' size=5>当月清收("+ckpm[i-1][1]+"元)</font></html>");
            ...
            for(int i=1;i<4;i++){
                ...
                label_3.setBounds(50, a+50, 100, 50);
                label_4= new JLabel("<html><font color='#FF0000' size=4>当月清收("+ckpm[ckpm.length-3+i-1][1]+"元)</font></html>");
                label_4.setBounds(120, a+50, 200, 50);
            ...
            timer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    try{
                        System.out.println(">>>>>>>>>>>客户经理清收不良贷款排名任务开始"+formatTemp.format(new Date()));
                        CockpitServiceIfc service = (CockpitServiceIfc) UIUtil
                                .lookUpRemoteService(CockpitServiceIfc.class);
                        ...
                        if(dk==null){
                            System.out.println(">>>>>>>>>>>客户经理清收不良贷款排名退出任务"+formatTemp.format(new Date()));
                            return;
                        }else{
                            //zzl 记录修改的中间表
                            String count=UIUtil.getStringValueByDS(null,"select name from s_count where name='客户经理清收不良贷款排名' and dates='"+DateUIUtil.getDateMonth(0,"yyyyMMdd")+"'");
                            if(count==null){
                                ...
                                for(int i=1;i<4;i++){
                                    ...
                                    jLabels[i-1][2].setText("<html><font color='#191970' size=5>当月清收("+ckpm[i-1][1]+"万元)</font></html>");
                                ...
                                for(int i=1;i<4;i++){
                                    ...
                                    jLabels[i+2][2].setText("<html><font color='#FF0000' size=4>当月清收("+ckpm[ckpm.length-3+i-1][1]+"万元)</font></html>");
                                    jLabels[i+2][3].setText("<html><font color='#FF0000' size=4>"+ckpm[ckpm.length-3+i-1][2]+"</font></html>");
                                }
                                UIUtil.executeUpdateByDS(null,"Update s_count set dates='"+DateUIUtil.getDateMonth(0,"yyyyMMdd")+"' where name='客户经理清收不良贷款排名'");
                                panel.updateUI();
                                System.out.println(">>>>>>>>>>>客户经理清收不良贷款排名退出任务"+formatTemp.format(new Date()));
                            }else{
                                System.out.println(">>>>>>>>>>>客户经理清收不良贷款排名退出任务"+formatTemp.format(new Date()));
