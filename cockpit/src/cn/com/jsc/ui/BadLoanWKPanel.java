...
public class BadLoanWKPanel {
    ...
    public JPanel getJLabel() {
        try {
            ...
            String [][] data=service.getgzhBlDKLvCount();
            panel.setLayout(null); //
            panel.setPreferredSize(new Dimension(1000, 1000)); //
            label_1 = new JLabel("<html><h3><font color='Black' size=6>"+DateUIUtil.getDateMonth(1,"yyyy年MM月dd日")+"各支行不良贷款情况统计</font></h3></html>");//
...
//                ImageIcon imgIcon= UIUtil.getImageFromServer("/applet/sw.gif");//
                ...
                label_3= new JLabel("<html><font color='#191970' size=5>"+data[i-1][0]+"</font></html>");
                label_3.setBounds(50, a+50, 100, 50);
                label_4= new JLabel("<html><font color='#191970' size=5>当前不良率("+data[i-1][1]+"%)</font></html>");
...
//                ImageIcon imgIcon= UIUtil.getImageFromServer("/applet/kq.png");//
                ...
                label_2.setBounds(0, a+50, 100, 50);
                label_3= new JLabel("<html><font color='#FF0000' size=4>"+data[data.length-3+i-1][0]+"</font></html>");
                label_3.setBounds(50, a+50, 100, 50);
                label_4= new JLabel("<html><font color='#FF0000' size=4>当前不良率("+data[data.length-3+i-1][1]+"%)</font></html>");
                label_4.setBounds(150, a+50, 200, 50);
...
//                panel.add(label_6);
            ...
            timer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    try{
                        System.out.println(">>>>>>>>>>>各支行不良贷款情况统计任务开始"+formatTemp.format(new Date()));
                        CockpitServiceIfc service = (CockpitServiceIfc) UIUtil
                                .lookUpRemoteService(CockpitServiceIfc.class);
                        ...
                        if(dk==null){
                            System.out.println(">>>>>>>>>>>各支行不良贷款情况统计退出任务无"+formatTemp.format(new Date())+"数据");
                            return;
                        }else {
                            //zzl 记录修改的中间表
                            String count = UIUtil.getStringValueByDS(null, "select name from s_count where name='各支行不良贷款情况统计' and dates='" + DateUIUtil.getDateMonth(0, "yyyyMMdd") + "'");
                            if (count == null) {
                                JLabel[][] jLabels = new JLabel[6][3];
                                JLabel JLabel = (javax.swing.JLabel) panel.getComponents()[0];
                                JLabel.setText("<html><h3><font color='Black' size=6>" + DateUIUtil.getDateMonth(1, "yyyy年MM月dd日") + "各支行不良贷款情况统计</font></h3></html>");
                                int row = 0;
                                int col = 0;
                                ...
                                for (int i = 1; i < 4; i++) {
                                    ...
                                    jLabels[i - 1][1].setText("<html><font color='#191970' size=5>"+data[i-1][0]+"</font></html>");
                                    jLabels[i - 1][2].setText("<html><font color='#191970' size=5>当前不良率("+data[i-1][1]+"%)</font></html>");
                                }
                                for (int i = 1; i < 4; i++) {
                                    jLabels[i + 2][0].setText("<html><font color='#FF0000' size=4>TOP"+(data.length-3+i)+"</font></html>");
                                    jLabels[i + 2][1].setText("<html><font color='#FF0000' size=4>"+data[data.length-3+i-1][0]+"</font></html>");
                                    jLabels[i + 2][2].setText("<html><font color='#FF0000' size=4>当前不良率("+data[data.length-3+i-1][1]+"%)</font></html>");
                                }
                                UIUtil.executeUpdateByDS(null, "Update s_count set dates='" + DateUIUtil.getDateMonth(0, "yyyyMMdd") + "' where name='各支行不良贷款情况统计'");
                                panel.updateUI();
                                System.out.println(">>>>>>>>>>>各支行不良贷款情况统计退出任务" + formatTemp.format(new Date()));
                            }else{
                                System.out.println(">>>>>>>>>>>各支行不良贷款情况统计退出任务" + formatTemp.format(new Date()));
                                return;
                            }
