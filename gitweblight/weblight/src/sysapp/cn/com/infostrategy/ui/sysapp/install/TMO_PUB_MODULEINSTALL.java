...
public class TMO_PUB_MODULEINSTALL extends AbstractTMO {
	...
	public HashVO getPub_templet_1Data() {
		...
		vo.setAttributeValue("templetcode", "MODULEINSTALL_CODE1"); //模版编码，请勿随便修改
		vo.setAttributeValue("templetname", "模块信息"); //模板名称
		vo.setAttributeValue("tablename", ""); //查询数据的表(视图)名
		vo.setAttributeValue("pkname", "c1"); //主键名
		vo.setAttributeValue("pksequencename", ""); //序列名
		vo.setAttributeValue("savedtablename", ""); //保存数据的表名
		vo.setAttributeValue("listcustpanel", null); //列表自定义面板
		vo.setAttributeValue("cardcustpanel", null); //卡片自定义面板
		vo.setAttributeValue("LISTROWHEIGHT", 100); //卡片自定义面板
		vo.setAttributeValue("TREEPK", "ID"); //卡片自定义面板
		vo.setAttributeValue("TREEISCHECKED", "Y"); //卡片自定义面板
		vo.setAttributeValue("TREEPARENTPK", "parentid"); //卡片自定义面板
		vo.setAttributeValue("TREEVIEWFIELD", "name"); //卡片自定义面板
	...
	public HashVO[] getPub_templet_1_itemData() {
		...
		itemVO.setAttributeValue("itemname", "图标"); //显示名称
		...
		itemVO.setAttributeValue("editformula", null); //编辑公式
		itemVO.setAttributeValue("defaultvalueformula", null); //默认值公式
		itemVO.setAttributeValue("listwidth", "136"); //列表是宽度
		itemVO.setAttributeValue("cardwidth", "150"); //卡片时宽度
		itemVO.setAttributeValue("listisshowable", "Y"); //列表时是否显示(Y,N)
		itemVO.setAttributeValue("listiseditable", "4"); //列表时是否可编辑,1-全部可编辑,2-仅新增可编辑,3-仅修改可编辑;4-全部禁用
		itemVO.setAttributeValue("cardisshowable", "Y"); //卡片时是否显示(Y,N)
		...
		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "version"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "版本信息"); //显示名称
		...
		itemVO.setAttributeValue("cardisshowable", "Y"); //卡片时是否显示(Y,N)
		itemVO.setAttributeValue("cardiseditable", "4"); //卡片时是否可编辑,1-全部可编辑,2-仅新增可编辑,3-仅修改可编辑;4-全部禁用
		itemVO.setAttributeValue("refdesc", "getCommUC(\"自定义控件\",\"卡片中的类\",\"\",\"列表渲染器\",\"cn.com.infostrategy.ui.sysapp.install.InstallScheduleTableRenderer\",\"列表编辑器\",\"cn.com.infostrategy.ui.sysapp.install.InstallScheduleTableEditor\");"); //参照定义
		vector.add(itemVO);

		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "date"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "发布日期"); //显示名称
		itemVO.setAttributeValue("itemtype", "文本框"); //控件类型
		itemVO.setAttributeValue("comboxdesc", null); //下拉框定义
		itemVO.setAttributeValue("refdesc", null); //参照定义
		itemVO.setAttributeValue("issave", "N"); //是否参与保存(Y,N)
		...
		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "control"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "操作"); //显示名称
		itemVO.setAttributeValue("itemtype", "自定义控件"); //控件类型
		itemVO.setAttributeValue("comboxdesc", null); //下拉框定义
		//		itemVO.setAttributeValue("refdesc", null); //参照定义
		itemVO.setAttributeValue("refdesc", "getCommUC(\"自定义控件\",\"卡片中的类\",\"\",\"列表渲染器\",\"cn.com.infostrategy.ui.sysapp.install.InstallScheduleTableRenderer\",\"列表编辑器\",\"cn.com.infostrategy.ui.sysapp.install.InstallScheduleTableEditor\");"); //参照定义
		...
		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "parentid"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "父亲节点"); //显示名称
		itemVO.setAttributeValue("itemtype", "文本框"); //控件类型
		itemVO.setAttributeValue("comboxdesc", null); //下拉框定义
		itemVO.setAttributeValue("refdesc", null); //参照定义
