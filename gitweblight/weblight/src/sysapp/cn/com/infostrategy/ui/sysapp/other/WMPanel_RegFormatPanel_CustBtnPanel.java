...
public class WMPanel_RegFormatPanel_CustBtnPanel extends AbstractCustomerButtonBarPanel implements ActionListener {
	...
	public void initialize() {
		...
		btn_1 = new WLTButton("预览"); //
		btn_2 = new WLTButton("导出SQL"); //
				...
				public void actionPerformed(ActionEvent e) {
					try {
						if (textArea == null || textArea.getText().trim().equals("") || textArea.getText() == null) {
							MessageBox.show(WMPanel_RegFormatPanel_CustBtnPanel.this,"请输入需要导入的注册样板!!");
							return;
						}
						...
						MessageBox.show(WMPanel_RegFormatPanel_CustBtnPanel.this,"导入XML格式注册样板成功!!!");
	...
	private void exportRegFormat() {
		...
		try{
			if(((AbstractStyleWorkPanel_02) this.getParentWorkPanel()).getBillListPanel().getSelectedBillVOs().length<=0){
				JOptionPane.showMessageDialog(this, "请先至少选择一条记录!");
				return;
			}
			BillVO[] billvo = ((AbstractStyleWorkPanel_02) this.getParentWorkPanel()).getBillListPanel().getSelectedBillVOs();
			
			String[] ids = new String[billvo.length];
			for(int i = 0;i<billvo.length;i++){
				ids[i]=billvo[i].getStringValue("id");
			}			
		FrameWorkMetaDataServiceIfc service = (FrameWorkMetaDataServiceIfc) UIUtil.lookUpRemoteService(FrameWorkMetaDataServiceIfc.class); //
		String sb_xml = service.exportXMLRegFormat(ids);
		BillDialog dialog = new BillDialog(this, "导出注册样板XML", 1000, 700);
