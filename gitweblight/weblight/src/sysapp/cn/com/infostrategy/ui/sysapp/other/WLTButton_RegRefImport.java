...
public class WLTButton_RegRefImport implements WLTActionListener {
	private JTextArea textArea = new JTextArea(); //

	public void actionPerformed(WLTActionEvent e) throws Exception {

		final BillDialog dialog = new BillDialog(e.getBillPanelFrom(), "导入XML", 1000, 700);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if (textArea == null || textArea.getText().trim().equals("") || textArea.getText() == null) {
						MessageBox.show(dialog, "请输入需要导入的注册参照!!");
						return;
					}
					...
					service.importXMLRegRef(textArea.getText());

					MessageBox.show(dialog, "导入XML格式注册参照成功!!!");
