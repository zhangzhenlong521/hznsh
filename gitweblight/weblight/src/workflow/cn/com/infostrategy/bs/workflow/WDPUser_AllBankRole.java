...
public class WDPUser_AllBankRole implements WorkflowDynamicParticipateIfc {
	...
	private String getDefineInfo() {
		return "动态参与者定义,范围:[全行],角色编码为空:[" + str_rolecode + "]"; //
	}
	...
	public WorkFlowParticipantBean getDynamicParUsers(String _loginuserid, BillVO _billvo, HashVO _dealpool, String code, String dealTyp, String code2, String _curractivityCode, String _curractivityName) throws Exception {
		...
		parBean.setParticipantDeptType("全行"); //

		String str_zh_dept_id = "";
		String str_zh_dept_code = "";
		String str_zh_dept_name = "全行";
		...
		if (str_rolecode == null || str_rolecode.trim().equals("")) { //如果不定义角色,则直接返回!!这时就等于是发邮件的样子了!!!
			parBean.setParticiptMsg("动态参与者定义,范围:[全行],角色定义为空:[" + str_rolecode + "],这样虽然没有直接找到人,但可以通过从部门选择人!"); //
			return parBean; //直接返回
		}
		...
		parBean.setParticiptMsg(getDefineInfo() + ",找到人员数量[" + hvs.length + "]"); //
		if (hvs != null && hvs.length > 0) {
			...
			for (int i = 0; i < userBeans.length; i++) {
				...
				userBeans[i].setSuccessParticipantReason("满足动态参与者条件,[全行],角色[" + str_rolename + "]"); ///
			}
