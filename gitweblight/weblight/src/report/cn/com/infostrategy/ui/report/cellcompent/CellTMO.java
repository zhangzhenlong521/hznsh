...
/**
 * 网格模板TMO，以前是内部类，因新增时也需要模板，故增加该类【李春娟/2012-04-12】
 */
public class CellTMO extends AbstractTMO {
	private static final long serialVersionUID = 8057184541083294474L;

	public HashVO getPub_templet_1Data() {
		HashVO vo = new HashVO(); //
		vo.setAttributeValue("templetcode", "CellTemplet"); //模版编码,请勿随便修改
		vo.setAttributeValue("templetname", "网格模板"); //模板名称
		vo.setAttributeValue("tablename", "pub_billcelltemplet_h"); //查询数据的表(视图)名
		vo.setAttributeValue("pkname", "id"); //主键名
		vo.setAttributeValue("pksequencename", "s_pub_billcelltemplet_h"); //序列名
		vo.setAttributeValue("savedtablename", "pub_billcelltemplet_h"); //保存数据的表名
		vo.setAttributeValue("listcustpanel", null); //列表自定义面板
		vo.setAttributeValue("cardcustpanel", null); //卡片自定义面板
		vo.setAttributeValue("cardcustpanel", null); //卡片自定义面板
		vo.setAttributeValue("isshowlistquickquery", "Y"); //保存数据的表名
		vo.setAttributeValue("ORDERCONDITION", "templetcode"); //排序条件【李春娟/2018-12-12】
	...
	public HashVO[] getPub_templet_1_itemData() {
		...
		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "rowlength"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "行数"); //显示名称
		itemVO.setAttributeValue("itemname_e", "rowlength"); //显示名称
		itemVO.setAttributeValue("itemtype", "文本框"); //控件类型
		itemVO.setAttributeValue("comboxdesc", null); //下拉框定义
		itemVO.setAttributeValue("refdesc", null); //参照定义
		itemVO.setAttributeValue("issave", "Y"); //是否参与保存(Y,N)
		...
		itemVO.setAttributeValue("loadformula", null); //加载公式
		itemVO.setAttributeValue("editformula", null); //编辑公式
		itemVO.setAttributeValue("defaultvalueformula", "15"); //默认值公式
		itemVO.setAttributeValue("listwidth", "250"); //列表是宽度
		itemVO.setAttributeValue("cardwidth", "150"); //卡片时宽度
		itemVO.setAttributeValue("listisshowable", "N"); //列表时是否显示(Y,N)
		itemVO.setAttributeValue("listiseditable", "1"); //列表时是否可编辑,1-全部可编辑,2-仅新增可编辑,3-仅修改可编辑;4-全部禁用
		...
		itemVO = new HashVO();
		itemVO.setAttributeValue("itemkey", "collength"); //唯一标识,用于取数与保存
		itemVO.setAttributeValue("itemname", "列数"); //显示名称
		itemVO.setAttributeValue("itemname_e", "collength"); //显示名称
		itemVO.setAttributeValue("itemtype", "文本框"); //控件类型
		itemVO.setAttributeValue("comboxdesc", null); //下拉框定义
		itemVO.setAttributeValue("refdesc", null); //参照定义
		itemVO.setAttributeValue("issave", "Y"); //是否参与保存(Y,N)
		...
		itemVO.setAttributeValue("loadformula", null); //加载公式
		itemVO.setAttributeValue("editformula", null); //编辑公式
		itemVO.setAttributeValue("defaultvalueformula", "8"); //默认值公式
		itemVO.setAttributeValue("listwidth", "250"); //列表是宽度
		itemVO.setAttributeValue("cardwidth", "150"); //卡片时宽度
		itemVO.setAttributeValue("listisshowable", "N"); //列表时是否显示(Y,N)
		itemVO.setAttributeValue("listiseditable", "1"); //列表时是否可编辑,1-全部可编辑,2-仅新增可编辑,3-仅修改可编辑;4-全部禁用
