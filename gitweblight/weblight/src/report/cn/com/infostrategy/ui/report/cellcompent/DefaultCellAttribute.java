...
public class DefaultCellAttribute implements CellAttribute, CellSpan {
	...
	private static final long serialVersionUID = -2906696400993114257L;
	...
	public void combine(int[] rows, int[] columns) {
		if (isOutOfBounds(rows, columns))
			return;
		int rowSpan = rows.length; //准备合并几行
		int columnSpan = columns.length; //准备合并几列
		int startRow = rows[0];
		...
		for (int i = 0, ii = 0; i < rowSpan; i++, ii--) { //处理合并范围的数据
