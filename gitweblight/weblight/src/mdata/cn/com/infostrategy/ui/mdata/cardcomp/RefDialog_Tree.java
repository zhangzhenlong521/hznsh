/**************************************************************************
 * $RCSfile: RefDialog_Tree.java,v $  $Revision: 1.9 $  $Date: 2012/09/14 09:22:57 $
 **************************************************************************/
...
/**
 * 树型模板参照,就是可以根据一个模板编码直接生成一个树型参照
 * 绝大多数树型参照都可以这么干!
 * 树型模板参照要实现以下几点功能:
 * 1.可以单选/多选,就是说可以反映定是否是勾选框方式！！
 * 2.可以指定返回的id,name的ItemKey,即用模板中的哪两个ItemKey作为id与name的返回字段!以前是约定死的，就是取第1个与第3个！不灵活！！
 * 3.以后可以继续扩展你机构选择一样，反映定某两个字段作为分类，然后快速选择！有点购物车的味道！
 * @author Administrator
 *
 */
public class RefDialog_Tree extends AbstractRefDialog {
	...
	public RefDialog_Tree(Container _parent, String _name, RefItemVO _refinitvalue, BillPanel _billPanel, CommUCDefineVO _dfvo) throws Exception {
	...
	protected void onConfirm() {
		...
		str_ref_name = str_name;//_vo.getStringValue(2); //str_name;//
	...
	private void showSQL() {
		...
		MessageBox.show(this, this.str_sql); //
	}
