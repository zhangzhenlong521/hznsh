 ...
 *
 */
public class RefDialog_RegFormat extends AbstractRegFormatRefDialog {
	private static final long serialVersionUID = -8323170455063789191L;

	private CommUCDefineVO dfvo = null; //

	public RefDialog_RegFormat(Container _parent, String _title, RefItemVO refItemVO, BillPanel panel, CommUCDefineVO _dfvo) {
			...
			if (getRefStyleType() == 0) {
				if (this.getBillFormatPanel().getReturnRefItemVOFrom() != null) { //如果定义了返回的面板类!则使用它试试
					...
					if (billPanel instanceof BillListPanel) {
						...
						return refItemvo;
					} else if (billPanel instanceof BillTreePanel) { //如果是树
						...
						if (this.getBillFormatPanel().getReturnRefItemVONameFieldName() != null && !this.getBillFormatPanel().getReturnRefItemVONameFieldName().equals("")) {
							BillVO[] billVOs = billTree.getSelectedParentPathVOs(); //
							StringBuffer sb_text = new StringBuffer(); //
							for (int i = 0; i < billVOs.length; i++) {
								sb_text.append(billVOs[i].getStringValue(this.getBillFormatPanel().getReturnRefItemVONameFieldName())); //
								if (i != billVOs.length - 1) {
									sb_text.append("->"); //
								}
							}
							refItemvo.setName(sb_text.toString()); //
						}
