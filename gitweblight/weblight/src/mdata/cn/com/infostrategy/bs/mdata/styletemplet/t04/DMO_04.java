/**************************************************************************
 * $RCSfile: DMO_04.java,v $  $Revision: 1.4 $  $Date: 2012/09/14 09:22:56 $
 **************************************************************************/
...
public class DMO_04 extends AbstractDMO{
	...
	public HashMap dealCommit(String _dsName, String _bsInterceptName, BillVO[] _insertobjs, BillVO[] _deleteobjs, BillVO[] _updateobjs) throws Exception {
		IBSIntercept_04 bsIntercept = getIntercept(_bsInterceptName); // 创建BS端拦截器
		...
		if (_updateobjs != null && _updateobjs.length > 0) {
			for (int i = 0; i < _updateobjs.length; i++) {
				...
				//					if (ld_version != null && getVersion(_dsName,_updateobjs[i]) != null && !ld_version.equals(getVersion(_updateobjs[i]))) {
	 ...
	 * @throws Exception
	 */
	private IBSIntercept_04 getIntercept(String _interceptName) throws Exception {
		if (_interceptName != null&&!_interceptName.equals( "" )) {
			return (IBSIntercept_04) Class.forName(_interceptName).newInstance();
		} else {
			return null;
		}
	}
 ...
 * *** empty log message ***
 *
 * Revision 1.3  2007/03/15 07:00:31  shxch
 * *** empty log message ***
 *
 * Revision 1.2  2007/03/09 01:17:20  sunxf
 * *** empty log message ***
 *
